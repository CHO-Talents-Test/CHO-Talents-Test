...
-- 찬성/반대 방향별 집계 없이, 등록 시점 과반 투표 수를 100%로 하는 진행률만 반환합니다.
...
BEGIN
  ...
  SELECT s.id, s.name, s.product_url, s.image_url, s.detail_image_url, s.description,
         ...
         CASE
           ...
           ELSE LEAST(100, ROUND((COALESCE(vc.vote_count, 0)::numeric / s.electorate_majority) * 100)::integer)
...
BEGIN
  ...
  SELECT q.id, q.name, q.product_url, q.image_url, q.detail_image_url, q.description, q.price_krw,
         ...
         CASE
           ...
           ELSE LEAST(100, ROUND((q.current_vote_count::numeric / q.electorate_majority) * 100)::integer)
         END,
